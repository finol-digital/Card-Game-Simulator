...
It provides permanent HTTP redirects for `/bluesky`, `/discord`, `/facebook`, `/games`, `/github`, `/play`, `/reddit`, `/twitter`, and `/x`, plus the legacy `/PRIVACY.html` URL; serves `llms.txt` as `text/markdown` when the homepage is requested with `Accept: text/markdown`; sets `Vary: Accept, Accept-Encoding` for both homepage variants; and gives markdown-capable clients a recoverable 404 response.
