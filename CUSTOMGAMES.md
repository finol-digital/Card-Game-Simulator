...
| CardProperties | List\<PropertyDef\> | [] | *PropertyDef* has the properties: *Name* and *Type*. *PropertyDef:Name* is a string indicating the name of the property, and this name can be referenced by CardImageURLName, CardPrimaryProperty, Enums, Extras, and HsdPropertyId. *PropertyDef:Type* can be **string**, **integer**, or **enum**. |
...
| Extras | List\<ExtraDef\> | [] | *ExtraDef* has the properties: *Property* and *Value*. *ExtraDef:Property* refers to a *PropertyDef:Name* in \<CardProperties\>. *ExtraDef:Value* is a string. If *Card:Properties\[ExtraDef:Property\]* equals *ExtraDef:Value*, then that card will be moved from the main deck and put into the extra deck. The **hsd** DeckFileType places heroes in extras, and the **ydk** DeckFileType places extra deck cards in extras. |
