## Purpose
Define ownership rules for `CgsNetPlayable` objects so spawned playables are owned by their creator, private playables are protected from non-owners, and sharing behavior is explicitly scoped to shared `CardStack` instances.
